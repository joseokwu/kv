...
export const Applied = ({ data, apply }) => {
    ...
    const notInteracted = useMemo(() => {
        return (
            ...
            data.filter(
                (item) =>
                    !item?.pendingRequests.find(
                        (i) => i.startupId === stateAuth?.user?.userId
                    ) &&
                    !item?.approvedRequests.find(
                        (i) => i.startupId === stateAuth?.user?.userId
                    )
            )
        );
        ...
        try {
            ...
            toast.error(
                err?.response?.data?.message ??
                    "There was an error sending this application"
            );
        }
    };

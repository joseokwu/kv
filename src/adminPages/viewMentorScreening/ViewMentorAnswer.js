...
export const ViewMentorAnswer = () => {
  ...
  const isInProgress = pathname.includes('inProgress');
  const isScheduled = pathname.includes('scheduled');
  ...
  return (
    <div className='p-5'>
      ...
      <section>
        {status === 'inProgress' ? (
          <EmptyState text={'Screening in Progress...'} />
        ) : status === 'scheduled' ? (
          <EmptyState text={'Screening has been Scheduled...'} />
        ) : (

...
export const CriteriaQuestions = () => {
    ...
    const addToCriteria = (index, data) => {
        ...
        console.log("categories", {
            noOfQuestion: categories[currentCategory].noOfQuestion,
            titleAndQuestion: categories[currentCategory].titleAndQuestion,
            name: categories[currentCategory].category,
        });
        console.log(currentCategory);
        console.log(categories);

        tempCriteria[currentCategory].titleAndQuestion[index] = data;
        tempCriteria[currentCategory].name =
            adminState?.categories[currentCategory].category;
        addNewCategory(tempCriteria);
        // {noOfQuestion,titleAndQuestion,category}
    };
                        ...
                        <article className={`mb-45 ${styles.tq_space}`}>
                            <div>
                                ...
                                <p className="mb-3">
                                    ...
                                    adipiscing elit. Condimentum turpis quisque
                                    ut.
                                </p>
                                <span className="d-flex align-items-center space-out border-bottom pb-4 mb-4">
                                    <img src={info} alt="info" />
                                    <p>
                                        All questions are rated on a scale of 0
                                        to 5 and a total of 30 questions.
                                    </p>
                                </span>

                                {categories?.length > 0 &&
                                    categories[currentCategory]
                                        ?.titleAndQuestion.length > 0 &&
                                    categories[
                                        currentCategory
                                    ]?.titleAndQuestion?.map((tq, i) => {
                                        console.log(categories);
                                        return (
                                            <TQItem
                                                data={tq}
